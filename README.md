...
3. **Synchronisation** : historique et dictionnaire reliés à un backend commun
   encore à concevoir; Neon, Supabase et les politiques de données restent des
   décisions ouvertes, et aucun de ces services n'est connecté au client Android
4. **Bulle flottante** 🧪 : le widget persistant façon Wispr Flow est implémenté
   dans la candidate alpha; sa matrice Gmail/ChatGPT/Claude et les longues
   dictées doivent encore être validées sur appareil
...
2. **Accessibilité** — c'est le mécanisme Android qui permet d'identifier et
   d'écrire dans le champ d'une autre application. Sans service réellement lié,
   le widget ne livre pas le texte à une cible externe : la transcription reste
   dans l'historique. Un repli presse-papiers n'est permis que si une cible
   capturée auparavant reste prouvée au moment de la livraison.
...
## Développer ou reprendre la candidate alpha

- [Index de la documentation technique](.docs/README.md)
- [Note de reprise autonome pour Claude](.docs/HANDOFF_CLAUDE.md)
- [Build, prérequis et état exact des preuves](.docs/BUILD_AND_VALIDATION.md)
- [Plan de validation sur téléphone](.docs/ANDROID_ALPHA_TEST_PLAN.md)

Pour reprendre la branche de travail sur un nouvel ordinateur :

```powershell
git clone --recurse-submodules --branch codex/android-alpha https://github.com/ReiiViilo/Chuchote-flow-android.git
```

La branche reste une candidate de développement. La page `Releases → latest`
est construite depuis `main` seulement et ne contient donc pas cette candidate
avant fusion.
...
> Sur un run de `main`, l'onglet
> [Actions](../../actions/workflows/build-apk.yml) publie aussi l'APK debug du
> même commit, sous forme d'artefact zippé réservé aux utilisateurs connectés.
> Sur une PR, cet artefact correspond à la PR tandis que `Releases → latest`
> reste sur `main`. Le workflow ne construit pas la variante QA : pour tester
> celle-ci, exécuter `assembleQa` depuis la branche.
...
Pour compiler localement ou produire la variante QA, suivre
[Build et validation Android](.docs/BUILD_AND_VALIDATION.md).
