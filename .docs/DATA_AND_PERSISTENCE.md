...
La synchronisation utilise un `SharedPreferences` nommé `chuchote_sync`, exclu des sauvegardes et transferts Android au même titre que `remote_transcription` : l'empreinte SHA-256 du dernier dictionnaire accepté par le relais configuré (`dictionary_signature` — liée à l'adresse de base du relais, effacée par toute mutation du dictionnaire, réinscrite seulement quand une republication complète est passée sans mutation concurrente) et la file des pierres tombales que le relais n'a pas encore acceptées (`pending_tombstones`, JSON d'objets `heard` / `replace_with` / `since`, donc des mots du dictionnaire personnel — alimentée seulement si la synchronisation a déjà été configurée une fois — drapeau `sync_configured` —, 200 paires au plus, rejouée 30 jours au plus). Un seul fil d'envoi ordonne tout ce qui part vers `/api/sync/dictionary` : une suppression ne peut plus être dépassée par une republication de démarrage encore en vol; un rejeu n'envoie que les pierres tombales inscrites avant sa demande (un mot supprimé, réappris puis resupprimé pendant que le fil est occupé part dans cet ordre); la pierre tombale est inscrite dans `chuchote_sync` **avant** le `DELETE` de `chuchote.db`, pour qu'une mort du processus entre les deux ne perde jamais la suppression (dans l'autre sens, une pierre tombale d'un mot encore vivant est écartée au rejeu suivant); une republication adresse tous ses lots au relais lu à son départ, et une série d'envois — lots, pierres tombales — s'arrête dès que ce relais n'est plus celui configuré (coupé, jeton tourné, adresse changée) : ce qui était en vol part, rien de plus, l'empreinte n'est pas inscrite et les pierres tombales restent en file. Un rejeu n'envoie et n'élague (paire redevenue vivante, péremption) que les pierres tombales inscrites à une génération **au plus égale** à celle observée à sa demande : celle d'une suppression demandée après lui est laissée à son propre travail. Frontière encore ouverte, sur décision d'Olivier : la borne est lue sous le verrou du coordinateur mais la mise en file se fait hors de lui, donc un rejeu demandé entre l'inscription d'une pierre tombale et la mise en file de son travail la verrait à sa propre génération et, la ligne n'étant pas encore effacée, l'élaguerait — inatteignable aujourd'hui (`synchroniserAuDemarrage` n'a qu'un appelant, l'init du magasin, sur le même fil que les suppressions), à fermer par `demander` sous le verrou ou par la boîte d'envoi transactionnelle; tout nouvel appelant de `synchroniserAuDemarrage` rouvre le sujet. Les écritures de `chuchote_sync` passent par `commit()` et son résultat : une pierre tombale qui n'a pas pu être inscrite — ou dont l'inscription a levé — part aussitôt, une seule fois, la suppression locale a lieu quand même et un avertissement est journalisé. Résiduels assumés de cet ordre, tous auto-réparés ou bornés : le processus meurt entre l'inscription et le `DELETE` (l'entrée est encore là au lancement suivant, la pierre tombale part puis la republication la rend vivante au relais); le `DELETE` échoue (disque, verrou SQLite) alors que la pierre tombale est déjà en file et partira (le desktop perd le mot jusqu'à la republication suivante, qui le rend); la pierre tombale peut atteindre le relais avant que le `DELETE` ait eu lieu (même réparation). L'empreinte n'est inscrite que si aucune mutation ne s'est glissée pendant les envois **et** qu'aucune pierre tombale n'est en file : une suppression en suspens n'est pas décrite par ce qui vient d'être publié, et le démarrage suivant republie plutôt que de laisser un mot vivant ici et mort au relais. Une pierre tombale n'est posée que pour la dernière ligne d'une paire (la table `dictionnaire` admet les doublons, la clé du relais est la paire) : supprimer un doublon n'annonce rien. Toute écriture refusée de la file est journalisée et n'altère pas ce que le coordinateur en sait; un mot réappris dont la pierre tombale n'a pas pu être retirée de la file est tenu pour vivant par tout rejeu jusqu'à ce qu'une écriture réussisse sans elle. Tout envoi attend au-delà des 30 s que le relais s'accorde (`SyncTimeouts`), pour qu'un délai côté client implique que le relais a renoncé — sans quoi un ajout expiré pourrait encore s'écrire après la pierre tombale envoyée ensuite; reste la fenêtre d'une requête SQL déjà partie quand la fonction est tuée, à fermer par des versions de mutation côté relais (décision ouverte, `Chuchote-Flow/.docs/OPEN_DECISIONS.md` D-006). Les pierres tombales ne sont pas liées à un relais : une suppression envoyée au relais du moment n'atteint pas un relais précédent. Références : [`SyncPusher.kt`](../app/src/main/kotlin/dev/soupslurpr/transcribro/remote/SyncPusher.kt) (l'appareil) et [`DictionarySyncCoordinator.kt`](../app/src/main/kotlin/dev/soupslurpr/transcribro/remote/DictionarySyncCoordinator.kt) (la politique, Kotlin pur).
